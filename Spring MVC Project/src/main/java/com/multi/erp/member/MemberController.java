...
// 컨트롤러에서 user라는 이름으로 Model객체에 저장된 어트리뷰트가 잇으면 이를 세션에 저장해준다. 
@Controller
@RequestMapping("/emp")
...
public class MemberController {
	...
	public ModelAndView login(MemberDTO loginUserInfo, HttpServletRequest request) {
		...
		/*
		 * loginUserInfo => 로그인을 요청한 사용자의 정보 (id, pass)저장되는 객체 user => 로글인을 성공한 후 리턴된는
		 * 객체
		 * 
		 */
		...
		// 1) 세션을 생성
		// => getSession()을 이용해서 세션을 생성: 세신이 이미 서버에 만들어진게 있으면 그 세션을 반환
		// 세션이 만들어진게 없으면 무조건 새로 생성
		String viewName = "";
		if (user != null) {
			// 로그인 성공
			HttpSession ssesion = request.getSession();
			ssesion.setAttribute("user", user);
	...
	public String logout(HttpSession session) {
	  ...
	  @RequestMapping("/spring/login.do") 
	  ...
	  public String springlogout(SessionStatus status) {
		  ...
		  status.setComplete();//세션에 잇는 user 객체를 제거하는 작업
		  return "redirect:/index.do";
	  }
