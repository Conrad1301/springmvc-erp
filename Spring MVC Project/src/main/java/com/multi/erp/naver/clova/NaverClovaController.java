...
public class NaverClovaController {
	//파일업로드 되는 영수증에 필요한 정보를 추출해서 리턴 - ocr서비스 사용후 결과를 뷰로 보내기
	@RequestMapping(value = "/naverclova/ocr", produces = "application/text;charset=utf-8")
	@ResponseBody
	public String ocrtest(NCPCloverOcrDTO dto, Model model,HttpSession session) throws IllegalStateException,IOException {
		...
		//ocr서비스와 연결
		String result = OCRIUtil.getJsonText(path+File.separator+storeFilename);
		System.out.println("=========================");
