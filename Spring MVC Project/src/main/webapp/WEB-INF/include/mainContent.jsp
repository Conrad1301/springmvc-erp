...
<script type="text/javascript">
	$(document).ready(function() {
		//alert("jquery");
		//id가 boardCategory인 엘리먼트의 모든 li 하위엘리먼트에 동일한 코드를 적용하겠다. 
		$("#boardCategory>li").each(function() {
			//현재 작업 중인 객체가 선택되면 실행
			$(this).click(function() {
				//alert("선택됨");
				category = $(this).text();// 현재 작업 중인 li의 text를 추출
				$("#boardCategory>li").removeAttr("class");//현재 작업 중인 li가 선택되도록 class 속성 설정
				...
				$.ajax({
					...
					error:function(a,b,c){//ajax실패시 원인(에러메세지)
						alert(c);
					}
				});//end ajax	
			});//end click
